...
struct _MateColorPickerPrivate {
...
static void mate_color_picker_destroy    (GtkObject             *object);
static void mate_color_picker_finalize   (GObject               *object);
static void mate_color_picker_clicked    (GtkButton             *button);
static void mate_color_picker_state_changed (GtkWidget *widget, GtkStateType previous_state);
static void mate_color_picker_realize (GtkWidget *widget);
static void mate_color_picker_style_set (GtkWidget *widget, GtkStyle *previous_style);
static void drag_data_get		(GtkWidget          *widget,
					 GdkDragContext     *context,
					 GtkSelectionData   *selection_data,
					 guint               info,
					 guint               time,
					 MateColorPicker   *cpicker);
static void drag_data_received		(GtkWidget        *widget,
					 ...
					 guint32           time,
					 MateColorPicker *cpicker);
static void mate_color_picker_set_property (GObject            *object,
					     guint               param_id,
					     const GValue       *value,
					     GParamSpec         *pspec);
static void mate_color_picker_get_property (GObject            *object,
					     guint               param_id,
					     GValue             *value,
					     GParamSpec         *pspec);
...
MATE_CLASS_BOILERPLATE (MateColorPicker, mate_color_picker,
...
mate_color_picker_class_init (MateColorPickerClass *class)
{
	...
	object_class->destroy = mate_color_picker_destroy;
	gobject_class->finalize = mate_color_picker_finalize;
	widget_class->state_changed = mate_color_picker_state_changed;
	widget_class->realize = mate_color_picker_realize;
	widget_class->style_set = mate_color_picker_style_set;
	button_class->clicked = mate_color_picker_clicked;
...
static void
render_dither (MateColorPicker *cp)
{
...
static void
mate_color_picker_realize (GtkWidget *widget)
{
	MateColorPicker *cp = MATE_COLOR_PICKER (widget);
...
static void
mate_color_picker_style_set (GtkWidget *widget, GtkStyle *previous_style)
{
	MATE_CALL_PARENT (GTK_WIDGET_CLASS, style_set,
				   (widget, previous_style));

	if (GTK_WIDGET_REALIZED (widget))
		render (MATE_COLOR_PICKER (widget));
}

static void
mate_color_picker_state_changed (GtkWidget *widget, GtkStateType previous_state)
{
		    ...
		    MateColorPicker *cpicker)
{
	...
	g_return_if_fail (MATE_IS_COLOR_PICKER (cpicker));
	...
	mate_color_picker_set_i16(cpicker, dropped[0], dropped[1],
				   dropped[2], dropped[3]);
		...
		MateColorPicker   *cpicker)
{
	...
	mate_color_picker_get_i16(cpicker, &r, &g, &b, &a);
...
static void
mate_color_picker_instance_init (MateColorPicker *cp)
{
	GtkWidget *alignment;
	GtkWidget *frame;
	...
	/* Create the widgets */
	cp->_priv = g_new0(MateColorPickerPrivate, 1);
...
static void
mate_color_picker_destroy (GtkObject *object)
{
	MateColorPicker *cp;
	...
	g_return_if_fail (MATE_IS_COLOR_PICKER (object));

	cp = MATE_COLOR_PICKER (object);
...
static void
mate_color_picker_finalize (GObject *object)
{
	MateColorPicker *cp;
...
/**
 * mate_color_picker_new
 *
 * Creates a new MATE color picker widget. This returns a widget in the form of a small button
 ...
 *
 * Returns:
 * Pointer to new MATE color picker widget.
 */

GtkWidget *
mate_color_picker_new (void)
{
	return g_object_new (MATE_TYPE_COLOR_PICKER, NULL);
}
...
cs_ok_clicked (GtkWidget *widget, gpointer data)
{
	...
	mate_color_picker_get_i16 (cp, &r, &g, &b, &a);
	g_signal_emit (cp, color_picker_signals[COLOR_SET], 0,
		       r, g, b, a);
}

static int
key_pressed (GtkWidget *widget, GdkEventKey *event, MateColorPicker *cp)
{
...
static void
mate_color_picker_clicked (GtkButton *button)
{
	MateColorPicker *cp;
	...
	g_return_if_fail (MATE_IS_COLOR_PICKER (button));

	cp = MATE_COLOR_PICKER (button);
...
/**
 * mate_color_picker_set_d
 * @cp: Pointer to MATE color picker widget.
...
void
mate_color_picker_set_d (MateColorPicker *cp, gdouble r, gdouble g, gdouble b, gdouble a)
{
	g_return_if_fail (cp != NULL);
	g_return_if_fail (MATE_IS_COLOR_PICKER (cp));
...
void
mate_color_picker_get_d (MateColorPicker *cp, gdouble *r, gdouble *g, gdouble *b, gdouble *a)
{
	g_return_if_fail (cp != NULL);
	g_return_if_fail (MATE_IS_COLOR_PICKER (cp));
...
void
mate_color_picker_set_i8 (MateColorPicker *cp, guint8 r, guint8 g, guint8 b, guint8 a)
{
	g_return_if_fail (cp != NULL);
	g_return_if_fail (MATE_IS_COLOR_PICKER (cp));
...
void
mate_color_picker_get_i8 (MateColorPicker *cp, guint8 *r, guint8 *g, guint8 *b, guint8 *a)
{
	g_return_if_fail (cp != NULL);
	g_return_if_fail (MATE_IS_COLOR_PICKER (cp));
...
void
mate_color_picker_set_i16 (MateColorPicker *cp, gushort r, gushort g, gushort b, gushort a)
{
	g_return_if_fail (cp != NULL);
	g_return_if_fail (MATE_IS_COLOR_PICKER (cp));
...
void
mate_color_picker_get_i16 (MateColorPicker *cp, gushort *r, gushort *g, gushort *b, gushort *a)
{
	g_return_if_fail (cp != NULL);
	g_return_if_fail (MATE_IS_COLOR_PICKER (cp));
...
void
mate_color_picker_set_dither (MateColorPicker *cp, gboolean dither)
{
	g_return_if_fail (cp != NULL);
	g_return_if_fail (MATE_IS_COLOR_PICKER (cp));
...
gboolean
mate_color_picker_get_dither (MateColorPicker *cp)
{
	g_return_val_if_fail (cp != NULL, FALSE);
	g_return_val_if_fail (MATE_IS_COLOR_PICKER (cp), FALSE);
...
/**
 * mate_color_picker_set_use_alpha
 * @cp: Pointer to MATE color picker widget.
...
void
mate_color_picker_set_use_alpha (MateColorPicker *cp, gboolean use_alpha)
{
	g_return_if_fail (cp != NULL);
	g_return_if_fail (MATE_IS_COLOR_PICKER (cp));
...
gboolean
mate_color_picker_get_use_alpha (MateColorPicker *cp)
{
	g_return_val_if_fail (cp != NULL, FALSE);
	g_return_val_if_fail (MATE_IS_COLOR_PICKER (cp), FALSE);
...
void
mate_color_picker_set_title (MateColorPicker *cp, const gchar *title)
{
	g_return_if_fail (cp != NULL);
	g_return_if_fail (MATE_IS_COLOR_PICKER (cp));
...
const char *
mate_color_picker_get_title (MateColorPicker *cp)
{
	g_return_val_if_fail (cp != NULL, NULL);
	g_return_val_if_fail (MATE_IS_COLOR_PICKER (cp), NULL);

	return cp->_priv->title;
}

static void
mate_color_picker_set_property (GObject            *object,
				 guint               param_id,
				 const GValue       *value,
				 GParamSpec         *pspec)
{
	MateColorPicker *self;
	gushort r, g, b, a;

	g_return_if_fail (object != NULL);
	g_return_if_fail (MATE_IS_COLOR_PICKER (object));

	self = MATE_COLOR_PICKER (object);

	switch (param_id) {
	case PROP_DITHER:
		mate_color_picker_set_dither(self, g_value_get_boolean (value));
		break;
	case PROP_USE_ALPHA:
		mate_color_picker_set_use_alpha(self, g_value_get_boolean (value));
		break;
	case PROP_TITLE:
		mate_color_picker_set_title(self, g_value_get_string (value));
		break;
	case PROP_RED:
		mate_color_picker_get_i16(self, &r, &g, &b, &a);
		mate_color_picker_set_i16(self,
...
static void
mate_color_picker_get_property (GObject            *object,
				 guint               param_id,
				 GValue             *value,
				 GParamSpec         *pspec)
{
	...
	switch (param_id) {
	case PROP_DITHER:
		g_value_set_boolean (value, mate_color_picker_get_dither(self));
		break;
	case PROP_USE_ALPHA:
		g_value_set_boolean (value, mate_color_picker_get_use_alpha(self));
		break;
	case PROP_TITLE:
		g_value_set_string (value, self->_priv->title);
		break;
	case PROP_RED:
		mate_color_picker_get_i16(self, &val, NULL, NULL, NULL);
		g_value_set_uint (value, val);
		break;
	case PROP_GREEN:
		mate_color_picker_get_i16(self, NULL, &val, NULL, NULL);
		g_value_set_uint (value, val);
		break;
	case PROP_BLUE:
		mate_color_picker_get_i16(self, NULL, NULL, &val, NULL);
		g_value_set_uint (value, val);
		break;
	case PROP_ALPHA:
		mate_color_picker_get_i16(self, NULL, NULL, NULL, &val);
		g_value_set_uint (value, val);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, param_id, pspec);
